...
{
    // ═══════════════════════════════════════════════════════════════════════════
    //  MANAGER REST (lo consume Universidad.razor).  Flujo del FRONT:
    //    Universidad.razor  →  UniversidadRSManager (aquí)
    //      →  HttpClientUtils<T> (Utils/HttpClientUtils.cs, get/post/put/delete)
    //        →  [HTTP JSON]  →  RestServicesCaso2/webresources/<recurso>  (FacultadRS, CursoRS, ...)
    //          →  BO  →  DAO  →  MySQL
    //  Cada método arma la URL del recurso y delega la llamada HTTP a HttpClientUtils.
    //  Los DTO (UniversidadDTO.cs) se emparejan con el JSON por el nombre de campo.
    public class UniversidadRSManager
    {
        // ⚠ Si el backend corre en EC2, cambia "localhost" por la IP pública de la instancia.
