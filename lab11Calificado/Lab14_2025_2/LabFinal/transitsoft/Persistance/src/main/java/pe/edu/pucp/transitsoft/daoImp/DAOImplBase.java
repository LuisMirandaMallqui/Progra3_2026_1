...
import pe.edu.pucp.transitsoft.dbmanager.DBManager;
...
public abstract class DAOImplBase {
    ...
    protected void abrirConexion() {
        this.abrirConexion(MotorDeBaseDeDatos.MYSQL);
    }
    
    protected void abrirConexion(MotorDeBaseDeDatos motor) {
        if (motor == MotorDeBaseDeDatos.MYSQL)
            this.conexion = DBManager.getInstanceMySQL().getConnectionMySQL();
        else
            this.conexion = DBManager.getInstanceMSSQL().getConnectionMSSQL();
    }
    ...
    public Integer retornarUltimoAutoGenerado() {
        ...
        try {
            String sql = DBManager.getInstanceMySQL().retornarSQLParaUltimoAutoGenerado();
            this.statement = this.conexion.prepareCall(sql);
            this.resultSet = this.statement.executeQuery();
    ...
    public List listarTodos() {        
        ...
        return this.listarTodos(MotorDeBaseDeDatos.MYSQL, sql, incluirValorDeParametros, parametros);
    }
    ...
    public List listarTodos(MotorDeBaseDeDatos motor, String sql, Consumer incluirValorDeParametros, Object parametros) {
        List lista = new ArrayList<>();
        try {
            this.abrirConexion(motor);
            if (sql == null) {
